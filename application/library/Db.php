...
Class Db
{
    ...
    public function __construct($db_config)
    {
        ...
        $this->is_log = $db_config["isLog"];
        if ($this->is_log)
        {
            if(!is_dir($db_config['logFilePath'])){
                mkdir ( $db_config['logFilePath'], '0777' );
            }
            $handle = fopen($db_config["logFilePath"] . date("Ym")."_dblog.txt", "a+");
            $this->handle = $handle;
        }
    }

    //选择数据库 跨数据库服务器可以配置在这更新
